...
/*
		console.log("test post_ content");
		console.dir(select("core/editor").getEditedPostContent());
		 		...
		 		if( index == 'post_title' ) {
		 			...
		 			if( is_gutenberg ) {
		 				...
		 				if( should_we_replace ) {
		 					...
		 					// magie !
		 					//wp.data.dispatch( 'core/editor' ).resetBlocks( wp.blocks.parse( new_value ) );

		 					var fixed_translation = original_translation;
		 					//fixed_translation = decodeEntities( fixed_translation );
		 					fixed_translation = unEscapeHTMLTags( fixed_translation );
		 					console.log("fixed = " + fixed_translation);
		 					console.log("replacing gutenberg content with fixed translation");
		 					//wp.data.dispatch( 'core/editor' ).resetBlocks( wp.blocks.parse( fixed_translation ) );
		 					wp.data.dispatch( 'core/block-editor' ).resetBlocks( wp.blocks.parse( fixed_translation ) );

		 					//console.log("replacing gutenberg content with ori translation");		 					wp.data.dispatch( 'core/editor' ).resetBlocks( wp.blocks.parse( original_translation ) );
		 				}
		 				...
		 				if( jQuery('textarea.editor-post-text-editor').val() !== undefined ) {
		 					...
		 					console.log("new content is " + new_value );
		 				...
		 				if( classic_editor_visual ) {
							...
							*/
							//jQuery( '#content_ifr' ).contents().find( '#tinymce' ).html( new_html );
			 				...
			 				var splitted = new_html.split("\n\n");
			 				var joined = '<p>' + splitted.join("</p>\n<p>") + '</p>';
			 				new_html = joined;
...
*/
							...
							editor.setContent(new_html);
		 					...
		 					// ajoute en 2020 mais en fait non 20210203 new_value = new_value.replace(/\n\s*\n/g, '\n');
